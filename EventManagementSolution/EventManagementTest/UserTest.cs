...
namespace EventManagementTest
{
    class UserTest
    {
        ...
        public async Task CreateNotification()
        {
            ...
            string SourceURL = "/api/notifications";

            await _notificationService.CreateNotification(UserId, Message, SourceURL);
        }
        ...
        {
            var notifications = await _notificationService.GetNotificationById(2, 1);
            Assert.IsNotNull(notifications);
        }

        [Test]
        public async Task GetNotificationByIdFail()
        {

            Assert.ThrowsAsync<NoNotificationFoundException>(async () =>
            {
                var notifications = await _notificationService.GetNotificationById(2, 9999);
            });
